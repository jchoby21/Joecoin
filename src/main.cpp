...
#if defined(NDEBUG)
# error "Joecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7e1fb2af8ebbe1f9ff06bde75938848ca1347e07e346dab29df8535634a44ebe");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Joecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Joecoin Signed Message:\n";
...
{
    // Joecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Joecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Joecoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Joecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Joecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Joecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe5894908d4e192544b38b0ff77cddf6c8791f7c908d8d6f3ab5683f4ab7e2f77");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "April 13th, Beginning of Easter Break -- Finally!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046671cb1ae41c5450078e8d022ab8de9e4133e7c616b94fc9bd34e11613d35836169f9507fdac05976d2d5090eee6c60c9359c33bad1dee13fa22f6f2cdd2cae0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1649808717; // 3
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084866233;

        if (fTestNet)
        {
            block.nTime    = 1649808691; // 2
            block.nNonce   = 385351599;
        }
...
//****** END OF NEW CODE PASTED IN
        ...
        assert(block.hashMerkleRoot == uint256("0x6fcd22430f757badd1c3aa0f3fd229655c148fffdca7b90d42b477d9a42da1f2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xc8, 0xb0, 0xdf }; // Joecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JoecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JoecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JoecoinMiner(CWallet *pwallet)
{
    printf("JoecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("joecoin-miner");
    ...
    try { loop {
        ...
        printf("Running JoecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JoecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JoecoinMiner, pwallet));
}
